...
class MLPAttentionLSTM(nn.Module):
    ...
    def __init__(self, n_timesteps=N_TIMESTEPS, n_sensors=N_SENSORS, hidden_size=96,
                 ...
                 n_continuous=38, match_params=True, n_cladding=4,
                 hrr_col=HRR_MW_COL):
        super().__init__()
        # Column of the design vector holding the normalised source strength.
        # Defaults to the DCLG layout; the Part1 corpus has a 13-column vector
        # and passes its own index.
        self.hrr_col = hrr_col

        # n_cladding defaults to 4, the DCLG corpus's system count, so existing
        # checkpoints load unchanged. The Part1 variant corpus carries nine
        # cladding cores and passes its own value.
    ...
    def forward(self, params, time_array, anchor):
        ...
        hrr_mw = params[:, self.hrr_col:self.hrr_col + 1] * HRR_MW_SCALE
