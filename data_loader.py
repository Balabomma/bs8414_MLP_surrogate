...
from config import (
    ...
    N_TIMESTEPS, N_SENSORS, N_SENSORS_RAW, TRAIN_RATIO, VALID_RATIO, TEST_RATIO,
...
def build_dataset(include_partial=False, min_timesteps=30):
    ...
    for sim in all_sims:
        ...
        if temps.shape[1] != N_SENSORS_RAW:
            skipped_bad_params += 1
            continue
        ...
        if not is_complete:
            ...
            padded = np.zeros((N_TIMESTEPS, N_SENSORS_RAW), dtype=np.float32)
